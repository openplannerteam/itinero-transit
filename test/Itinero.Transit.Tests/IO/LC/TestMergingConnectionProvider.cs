...
//            //Log($"NMBS Table: {sncbTt.StartTime():HH:mm} --> {sncbTt.EndTime():HH:mm}, {sncbTt.Connections().Count()} entries");
//            var deLijnTt = deLijn.GetTimeTable(moment);
//            //Log($"De Lijn Table: {deLijnTt.StartTime():HH:mm} --> {deLijnTt.EndTime():HH:mm}, {deLijnTt.Connections().Count()} entries");
//
//            //Log($"Synthetic table with {graph.Count} entries,  starting at {tt.StartTime()} till {tt.EndTime()}");
