...
namespace Itinero.Transit
{
    ...
    public class OsmTransferGenerator<T> : WalksGenerator<T> where T : IJourneyStats<T>
    {
        ...
        private Journey<T> CreateInternalTransfer(Journey<T> buildOn, ulong time)
        {

            ulong timeDiff;
            if (time < buildOn.Time)
            {
                timeDiff = buildOn.Time - time;
            }
            else
            {
                timeDiff = time - buildOn.Time;
            }
        ...
        public Journey<T> CreateDepartureTransfer(Journey<T> buildOn, uint nextConnection, ulong connDeparture, ulong connDepartureLoc)
        {
            throw new NotImplementedException();
        }

        public Journey<T> CreateArrivingTransfer(Journey<T> buildOn, uint previousConnection, ulong connArrival, ulong connArrivalLoc)
        {
            throw new NotImplementedException();
        }
    }
